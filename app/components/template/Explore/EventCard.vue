...
const props = defineProps<{
  event: KEvent | null,
  date: KDateApi,
  // `fluid` : la carte prend toute la largeur de sa cellule (grille fluide) au lieu de 13 rem.
  fluid?: boolean,
}>()
</script>

<template>
  <div class="flex flex-col min-w-0 bg-primary-700-kv3 hover:bg-primary-900-kv3 rounded-lg cursor-pointer" :class="fluid ? 'w-full h-full' : 'w-52 h-fit'" v-if="event">
    <EventVisual class="w-full h-28 sm:h-32 rounded-t-lg object-cover" :event="event"></EventVisual>
    <div class="p-2.5 min-w-0">
      <div class="font-semibold text-base md:text-lg text-primary-100-kv3 line-clamp-2">{{ event.name }}</div>
      <div class="font-normal text-sm md:text-base text-primary-100-kv3 flex flex-row items-start space-x-1 mt-1">
        <svg class="w-4 h-4 shrink-0 mt-0.5" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <span class="capitalize leading-tight">
          {{ DateTime.fromISO(date.startDateTime).setLocale('fr-FR').toLocaleString({weekday: 'long', month: 'long', day: 'numeric'}) }}
        </span>
      </div>
      <div class="font-normal text-sm md:text-base text-primary-100-kv3 flex flex-row items-center space-x-1">
