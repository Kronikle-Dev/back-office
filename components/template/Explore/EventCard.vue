...
<template>
  <div class="flex flex-col w-52 bg-primary-700-kv3 hover:bg-primary-900-kv3 rounded-lg cursor-pointer" v-if="event">
    <img class="h-28 rounded-t-lg" :src="event.imageUrl"/>
    <div class="p-2.5">
      ...
      <div class="font-normal text-lg text-primary-100-kv3 flex flex-row items-center space-x-1">
        ...
        <span class="capitalize">
          {{ DateTime.fromISO(date.startDateTime).toLocaleString({weekday: 'long', month: 'long', day: 'numeric'}) }}
        </span>
      </div>
      <div class="font-normal text-lg text-primary-100-kv3 flex flex-row items-center space-x-1">
        ...
        <span class="capitalize">
          {{ DateTime.fromISO(date.startDateTime).toLocaleString({hour: '2-digit', minute: '2-digit'}) }} - {{ DateTime.fromISO(date.endDateTime).toLocaleString({hour: '2-digit', minute: '2-digit'}) }}
        </span>
      </div>
    </div>
  </div>
